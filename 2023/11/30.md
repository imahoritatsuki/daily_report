...
## わかったこと・復習になったこと
  ...
  - Supabaseには１時間あたりに送信できるメール数に制限があるので、開発および本番環境ではカスタム SMTP プロバイダーを使用してメールを送信する必要がある。
    - SendGrid、Mailgun、Amazon SESなどが一般
    - 今回はGmailでテスト
  - 登録済みメールアドレスでサインアップしてくると、`supabase.auth.signUp`の`data.user?.identities`が空で帰ってくる
...
## 感じたこと
- コードを改めて整理して、デプロイをしなおした。
- Supabaseをでのログイン機能を実装したテンプレートも作ることができたので、今後のアプリケーション作成にも活かせると思う。
- Zodはめっちゃ簡単にバリデーションを実装できる。他のライブラリやツールを使ってみて、違いも理解したいところ。
