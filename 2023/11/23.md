...
## 感じたこと
- Supabaseの認証周りの進捗が良くないので、コードの整理も兼ねて、UIの作成を先に進める。
- やはり個人的には、UIを先に作ってしまった方が色々考えやすい気がする。
- 少なくとも今回はUIがほとんど決まってるので。
- heightやwidthの指定が、コンポーネントが深くなっていくにつれて、イメージしているようにかけられないことが多い。
- Nextjsのレンダリングの種類は肝の部分なので、改めて学習しないといけないな。
