...
- ブログ執筆
  - [Next.js（App Router） と NotionAPI で簡単なブログサイトを構築](https://zenn.dev/jinku/articles/722e8f93e87111)
...
## わかったこと・復習になったこと
  ...
  - `n2m.toMarkdownString`でページのブロックを全て一括で取ってきて、`n2m.pageToMarkdow`でそれをさらに一つにまとめるのが良いと思っていたが、一つひとつのブロックにスタイルを当てることができないので、mapで展開する方が良さそう。
  - Tailwindcssは番号付きリストの番号を描画するには、それようにクラスを当てる必要がある！！
  - react markdownでクラスを当てるには`components={{}}`でカスタムする必要がある。
...
## 感じたこと
- まさかのTailwindcssは番号付きリストの番号をデフォルトでは表示しなかった！
- 利用するcssフレームワークによってこういった挙動に違いがあると思うので、それぞれの仕様の違いを意識する必要がある。
- ブログも書けた。
- 作ってみたいアプリができたので、明日からまた調べてみる。
