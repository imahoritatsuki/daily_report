...
## わかったこと・復習になったこと
  ...
  - NextjsとRailsを一つの親ディレクトリと一つのGitHubで管理する場合は、各.gitディレクトリを削除し、親ディレクトリで`git init`を行う
  - @notionhq/clientを使って簡単にデータの取得ができる。
  - 
...
## 感じたこと
- NotionAPi、もしかしてfetchの方が容易だったりするか？？
- Markdownはライブラリを入れてページに描画する必要があるので、明日はそこを確認して仕上げる。
- Notionはページの本文などを取ってくる時は、PageではなくBlockになる。
- 簡単なページなら結構すぐに作れるようになってきた。Tailwindcssも結構慣れてきた感じがある。
- いくつかの企業様に応募した。
