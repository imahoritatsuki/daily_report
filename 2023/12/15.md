...
## わかったこと・復習になったこと
  ...
  - generateMetadata による静的メタデータと動的メタデータの両方は、Server Components でのみサポートされる。
  - GTMでのGA4の設定方法（主に名称周り）が結構変更されていた
...
## 感じたこと
- GTMの設定自体は難しくなかったが、GA4のUI上、サイトの各ページ名を動的に変更できる方が良さそう。
- まあGA4を使わないにしても、そうであるべきだった。
- ページタイトルを動的に変更しようと思うと、Page.tsxはサーバーコンポーネントで作るのが原則になるかな？
